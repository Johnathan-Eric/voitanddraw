...
function print_get_content($printer_info, $orderid) {
	global $_W;
	$order_info = pdo_get('storex_order', array('id' => $orderid), array('remark', 'time', 'sum_price', 'mobile', 'static_price', 'hotelid', 'roomid', 'ordersn', 'cprice', 'nums', 'day', 'contact_name', 'mode_distribute', 'addressid', 'order_time', 'spec_info'));
	if (empty($order_info)) {
		return array();
	}
	$order_info['spec_info'] = iunserializer($order_info['spec_info']);
	$storeinfo = pdo_get('storex_bases', array('id' => $order_info['hotelid']), array('id', 'store_type', 'title'));
	...
	$spec_val = !empty($order_info['spec_info']['goods_val']) && implode(' ', $order_info['spec_info']['goods_val']) ? implode(' ', $order_info['spec_info']['goods_val']) : '暂无';
...
function print_get_address($addressid) {
	$address = '';
	if (!empty($addressid)) {
		$address_info = pdo_get('storex_member_address', array('id' => $addressid), array('province', 'city', 'district', 'address'));
...
function print_get_mode($storeid) {
	global $_W;
