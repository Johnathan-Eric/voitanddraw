...
function entry_fetch($storeid, $type, $params) {
	...
	} elseif ($type == 'bargain_activity') {
		$entry_url = murl('entry', array('do' => 'display', 'm' => 'wn_storex', 'id' => $storeid), true, true) . '#/Bargain/Info/' . $params['bargain_list_id'];
	}
		...
		foreach ($article as $val) {
			if (!empty($wxapp)) {
				$article_entry_routes[] = array(
					'type' => 'article',
					'name' => $val['title'],
					'link' => '/Notice/' . $storeid . '/' . $val['id'] . '/article',
				);
			} else {
...
function usercenter_entry_fetch($storeid, $params = array()) {
	...
	$usercenter_entry_routes = array(
		...
		array(
			'type' => 'orderlist',
			'name' => '订单中心',
			'link' => $url . '#/Home/OrderList?type=all',
		),
		array(
			'type' => 'mycouponlist',
			'name' => '我的卡券',
		...
		array(
			'type' => 'credit',
			'name' => '我的余额',
			'link' => $url . '#/Home/Credit?type=credit2',
		),
		array(
...
function wxapp_usercenter_entry($storeid, $params = array()) {
	$usercenter_entry_routes = array(
		array(
			'type' => 'usercenter',
			'name' => '个人中心',
		...
		array(
			'type' => 'recharge_credit',
			'name' => '余额充值',
			'link' => '/Home/Recharge/credit',
		),
		array(
			'type' => 'creditsrecord',
			'name' => '余额记录',
			'link' => '/Home/CreditsRecord',
		),
		array(
			'type' => 'booking',
			'name' => '预约表单',
			'link' => '/Home/BookingList',
		),
		array(
			'type' => 'map',
			'name' => '导航',
			...
			foreach ($category as $key => &$info) {
				if (empty($info['parentid'])) {
					...
					if ($info['category_type'] == 1) {
						$vue_route = '/Category/HotelList/' . $storeid . '/' . $info['id'];
						...
						if (!empty($wxapp)) {
							if (empty($_W['wn_storex']['store_info']['store_type'])) {
								...
								$vue_route = '/Store/GoodList?id=' . $storeid . '&sub_id=' . $info['id'];
							}
						} else {
							if (empty($_W['wn_storex']['store_info']['store_type'])) {
								$vue_route = '/Category/' . $storeid . '?cid=' . $info['id'];
					...
					if (!empty($wxapp)) {
						$category_list[$info['parentid']]['group'][$key]['link'] = '/Store/GoodList?id=' . $storeid . '&sub_id=' . $info['id'];
					} else {
						$category_list[$info['parentid']]['group'][$key]['link'] = '/Store/GoodList?id=' . $storeid . '&sub_id=' . $info['id'];
					}
				}
