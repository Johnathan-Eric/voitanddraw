...
function clerk_order_operation($order, $store_type) {
	...
	if ($order['status'] == ORDER_STATUS_CANCEL || $order['status'] == ORDER_STATUS_REFUSE || $order['status'] == ORDER_STATUS_OVER) {
		...
		if ($order['paystatus'] == PAY_STATUS_PAID) {
			if ($store_type == STORE_TYPE_HOTEL) {
				...
				if ($room['is_house'] == 1) {
					if (($order['goods_status'] == GOODS_STATUS_NOT_CHECKED || empty($order['goods_status']))) {
						$order['is_access'] = 1;
					}
					$order['is_assign'] = 1;
				}
