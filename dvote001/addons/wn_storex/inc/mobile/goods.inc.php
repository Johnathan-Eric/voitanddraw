...
if (!empty($_GPC['group_id'])) {
	...
	$result = plugin_group_status($group_id);
	if (is_error($result)) {
...
if ($op == 'goods_info') {
	...
	if ($store_info['store_type'] == 1) {
		if ($goods_info['is_house'] == 1) {
			$goods_info = room_params($goods_info);
			...
			$goods_info = room_calcul_sumprice($dates, $search_data, $goods_info);
		}
	} else {
		$visit = pdo_get('storex_goods_visit', array('id' => $goodsid, 'openid' => $_W['openid']));
		if (empty($visit)) {
			$goods_visit = array(
				'uniacid' => $_W['uniacid'],
				'storeid' => $store_id,
			...
			pdo_insert('storex_goods_visit', $goods_visit);
			pdo_update('storex_goods', array('visit_times +=' => 1), array('id' => $goodsid));
		}
		if ($goods_info['isrecommend'] == 1) {
			$goods_info['attr'][] = '推荐';
		}
		if ($goods_info['isnew'] == 1) {
			$goods_info['attr'][] = '新品';
		}
		if ($goods_info['ishot'] == 1) {
			$goods_info['attr'][] = '热卖';
		}
		if ($goods_info['isnopostage'] == 1) {
			$goods_info['attr'][] = '包邮';
		}
	...
	if ($store_info['agent_status'] == 1 && !empty($agent_info)) {
		...
		if (!empty($ratio)) {
			...
			$goods_info['agent']['agentid'] = $agent_info['id'];
		}
	}
	...
	if ($store_info['store_type'] != 1) {
		...
		if (!empty($sales_packages) && is_array($sales_packages)) {
			foreach ($sales_packages as &$package) {
	...
	//砍价活动
	$goods_info['bargain'] = array();
	$bargainid = intval($_GPC['bargainid']);
	if (!empty($bargainid) && plugin_exists('wn_storex_plugin_bargain')) {
		$bargain = pdo_get('storex_plugin_bargain', array('id' => $bargainid));
		if (!empty($bargain) && $bargain['starttime'] <= TIMESTAMP && TIMESTAMP < $bargain['endtime'] && $bargain['stock'] > 0) {
			$bargain['starttime'] = date('Y-m-d H:i:s', $bargain['starttime']);
			$bargain['endtime'] = date('Y-m-d H:i:s', $bargain['endtime']);
			$bargain['bargain_total'] = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('storex_plugin_bargain_list') . " WHERE bargainid = :bargainid", array(':bargainid' => $bargain['id']));
			$goods_info['bargain'] = $bargain;
		}
	}
...
if ($op == 'spec_info') {
	...
	$spec_info['cprice'] = sprintf("%.2f", $discount_value * $spec_info['cprice']);
	if (!empty($spec_goods)) {
		$spec_goods['cprice'] = sprintf("%.2f", $discount_value * $spec_goods['cprice']);
		$spec_info = array(
...
if ($op == 'info') {
	...
	$bargain_list_id = intval($_GPC['bargain_list_id']);
	$order_goods = array();
	if ($store_info['store_type'] == STORE_TYPE_HOTEL) {
		...
		if (!empty($goods) && is_array($goods)) {
			...
			if (!empty($bargain_list_id)) {
				$order_goods[0]['cprice'] = get_bargain_info($store_id, $bargain_list_id);
			}
		} else {
		...
		if ($_GPC['is_cart'] == 1) {
			...
			if (empty($infos['express']['condition'])) {
				$infos['express']['express'] = $infos['express']['default_express'];
			}
		}
	}
	$address = pdo_getall('storex_member_address', array('uid' => $uid, 'uniacid' => intval($_W['uniacid'])));
	...
	if (empty($group) && empty($bargain_list_id)) {
		if ($store_info['market_status'] == 1) {
			...
			$discount = card_group_discount($store_id);
// 			discount_type  1满减  2打折
			if (!empty($discount)) {
				if ($discount['discount_type'] == 1) {
					$discount_info['discount_type'] = $discount['discount_type'];
					$discount_info['condition'] = $discount['condition_1'];
					$discount_info['discount'] = $discount['discount_1'];
				} elseif($discount['discount_type'] == 2) {
					$discount_info['discount_type'] = $discount['discount_type'];
					$discount_info['condition'] = $discount['condition_2'];
					$discount_info['discount'] = $discount['discount_2'];
				}
	...
	if (!empty($comment_list) && is_array($comment_list)) {
		...
		foreach ($comment_list as $key => &$value) {
			if (empty($value['thumb'])) {
				$fans_info = mc_fansinfo($value['uid']);
				$value['thumb'] = tomedia($fans_info['headimgurl']);
			} else {
				$value['thumb'] = tomedia($value['thumb']);
			}
...
function get_order_goods($store_info, $goods, $group = array()) {
	global $_W, $_GPC;
	$discount_value = goods_member_get_discount($store_info['id']);
...
function goods_hotel_order($insert, $store_info, $uid, $selected_coupon = array()) {
	...
	if (!empty($order_id)) {
		...
		if (empty($_GPC['wxapp']) && empty($_GPC['v'])) {
		...
		if (plugin_exists('wn_storex_plugin_printer')) {
			$mode_info = print_get_mode($store_info['id']);
			if (!empty($mode_info['defaultid']) && in_array('1', $mode_info['print_mode'])) {
				print_order($mode_info['defaultid'], $order_id, $store_info['id']);
			}
		}
...
function goods_common_order($insert, $store_info, $uid, $selected_coupon = array(), $group = array()) {
	...
	if ($_GPC['is_cart'] == 1) {
		...
		foreach ($order_goods as $val) {
			...
			if (!empty($val['id'])) {
				$cart_good['id'] = $val['id'];
				if (intval($_GPC['order']['mode_distribute']) == 2 && !express_goods_dispatch($val['id'], $_GPC['citycode'])) {
					continue;
				}
			}
		...
		if (!empty($bargain_list_id)) {
			$goods_info['cprice'] = get_bargain_info($store_info['id'], $bargain_list_id);
			$insert['bargainid'] = $bargain_list_id;
		}
		$cart_good = array();
		$spec = array();
	...
	if ($_GPC['is_cart'] == 1) {
		...
		//计算运费
		if ($insert['mode_distribute'] == 2) {
			$insert = calculate_express($goods_info, $insert, $_GPC['citycode']);
		}
	...
	if (!empty($group)) {
		...
		$result = plugin_group_status($_GPC['group_id']);
		if (is_error($result)) {
			wmessage($result, '', 'ajax');
		}
	}
	...
	if (!empty($order_id)) {
		...
		if (!empty($bargain_list_id)) {
			pdo_update('storex_plugin_bargain_list', array('status' => 2), array('id' => $bargain_list_id, 'uniacid' => $_W['uniacid'], 'storeid' => $store_info['id'], 'openid' => $_W['openid']));
			get_bargain_info($store_info['id'], $bargain_list_id, true);
		}
		check_member_exist($insert, $uid);
		insert_order_success($order_id, $uid);
...
// 			storex_send_email($store_info, $insert, $goods_info);
		...
		if (empty($_GPC['wxapp']) && empty($_GPC['v'])) {
			storex_send_notice($store_info, $order_id);
			if ($_GPC['is_cart'] != 1) {
				storex_send_notice_touser($insert, $store_info, $goods_info);
			}
		}
...
function get_info_by_addressid($insert) {
	if (!empty($insert['addressid'])) {
		$address = pdo_get('storex_member_address', array('id' => $insert['addressid']), array('id', 'username', 'mobile'));
		if (!empty($address)) {
...
function check_member_exist($insert, $uid) {
	...
	$member = array(
		...
		'nickname' => $insert['contact_name'],
		'mobile' => $insert['mobile'],
	);
	if (!member_single($member)) {
		member_insert($member);
	} else {
		$storex_member = pdo_get('storex_member', array('weid' => $_W['uniacid'], 'from_user' => $_W['openid']), array('id', 'realname', 'mobile', 'nickname'));
		if (!empty($storex_member)) {
			$update = false;
			if (empty($storex_member['realname'])) {
				$storex_member['realname'] = $insert['contact_name'];
				$update = true;
			}
			if (empty($storex_member['mobile'])) {
				$storex_member['mobile'] = $insert['mobile'];
				$update = true;
			}
			if (empty($storex_member['nickname'])) {
				$storex_member['nickname'] = $insert['contact_name'];
				$update = true;
			}
			if (!empty($update)) {
				pdo_update('storex_member', $storex_member, array('id' => $storex_member['id']));
			}
		}
	}
}
...
function room_order($insert, $goods_info, $orderid = '') {
	...
	if (!empty($orderid)) {
		$status = room_check_assign($insert, array($insert['roomitemid']), true);
		if (empty($status)) {
	...
	room_check_nums($dates, $search_data, $goods_info);
	$goods_info = room_calcul_sumprice($dates, $search_data, $goods_info);
...
function calcul_discounts_price($insert, $store_info, $uid, $selected_coupon = array()) {
	if ($store_info['market_status'] == 1) {
		...
		if ($selected_coupon['type'] == 3) {
			...
			$insert['sum_price'] = card_discount_price($store_info['id'], $insert['sum_price']);
		}
	}
	return $insert;
}
...
function storex_send_email($store_info, $insert, $goods_info) {
	...
	if (!empty($emails) && is_array($emails)) {
		foreach ($emails as $mail) {
			if (!preg_match(REGULAR_EMAIL, $mail)) {
				continue;
			}
...
function storex_send_sms($insert, $store_info) {
    ...
    if (!empty($clerks) && is_array($clerks)) {
    	foreach ($clerks as $k => $val) {
    		if (!in_array($val['mobile'], $phones) && preg_match(REGULAR_MOBILE, $val['mobile'])) {
    ...
    if (!empty($phones)) {
    	...
    	$plugin_exists = plugin_exists('wn_storex_plugin_sms');
    	foreach($phones as $phone) {
    		$result = '';
    		if ($plugin_exists) {
    			$result = sms_send($phone, $content, 'clerk');
    		}
    		sms_send_cloud_system($phone, $content, 'clerk', $result);
    	}
    }
}
...
function storex_send_notice($store_info, $orderid = '') {
	if (!empty($store_info['openids']) && is_array($store_info['openids'])) {
		foreach ($store_info['openids'] as $openid) {
			if (!empty($orderid)) {
				$info = '店铺有续订的订单,为保证用户体验度，请及时处理! 支付状态：未支付！';
			} else {
				$info = '店铺有新的订单,为保证用户体验度，请及时处理! 支付状态：未支付！';
			}
...
function get_bargain_info($store_id, $bargain_list_id, $update_stock = false) {
	global $_GPC, $_W;
	if (!empty($bargain_list_id)) {
		$bargain_list = pdo_get('storex_plugin_bargain_list', array('id' => $bargain_list_id, 'uniacid' => $_W['uniacid'], 'storeid' => $store_id, 'openid' => $_W['openid']));
		if (!empty($bargain_list)) {
			$bargain = pdo_get('storex_plugin_bargain', array('id' => $bargain_list['bargainid']));
			if (!empty($update_stock)) {
				pdo_update('storex_plugin_bargain', array('stock' => $bargain['stock'] - 1), array('id' => $bargain['id']));
			} else {
				if ($bargain['stock'] <= 0) {
					pdo_update('storex_plugin_bargain', array('status' => 3, 'reason' => '库存告罄'), array('id' => $bargain['id']));
					wmessage(error(-1, '很遗憾，该砍价商品库存告罄'), '', 'ajax');
				}
				if ($bargain_list['status'] == 0) {
					wmessage(error(-1, '砍价未完成'), '', 'ajax');
				}
				if ($bargain_list['status'] == 2) {
					wmessage(error(-1, '砍价已下单'), '', 'ajax');
				}
				if ($bargain_list['status'] == 3) {
					wmessage(error(-1, '砍价失败，不能下单'), '', 'ajax');
				}
				return $bargain_list['cprice'];
			}
		} else {
			wmessage(error(-1, '不存在的砍价'), '', 'ajax');
		}
	}
}
