...
$ops = array('display', 'register', 'agent_team', 'apply', 'apply_list', 'isagent');
...
if ($op == 'display') {
	...
	if ($register_info['status'] == 2) {
		...
		$second_agent = pdo_getall('storex_agent_apply', array('pid' => $register_info['id']), array('id'));
		if (!empty($second_agent)) {
			//二级分销的订单
			$second_order = pdo_getall('storex_order', array('agentid' => $second_agent, 'status' => array(0, 1, 3)), array('id', 'roomid', 'sum_price', 'time', 'nums', 'is_package', 'cart', 'agentid', 'cprice', 'status'), '', 'time DESC');
			if (!empty($second_order)) {
				$second_order_info = order_goods_infos($second_order, $table, 2);
				$agent_log = array_merge($agent_log, $second_order_info);
			}
			$third_agent = pdo_getall('storex_agent_apply', array('pid' => $second_agent), array('id'));
			if ($third_agent) {
				//三级分销的订单
				$third_order = pdo_getall('storex_order', array('agentid' => $third_agent, 'status' => array(0, 1, 3)), array('id', 'roomid', 'sum_price', 'time', 'nums', 'is_package', 'cart', 'agentid', 'cprice', 'status'), '', 'time DESC');
				if (!empty($third_order)) {
					$third_order_info = order_goods_infos($third_order, $table, 3);
					$agent_log = array_merge($agent_log, $third_order_info);
				}
			}
		}
...
if ($op == 'isagent') {
	$agentinfo = pdo_getall('storex_agent_apply', array('openid' => $_W['openid']), array('id', 'storeid'), 'storeid');
	$agent = array();
	if (!empty($agentinfo)) {
		foreach ($agentinfo as $k => $info) {
			$agent[$k] = $info['id'];
		}
	}
	wmessage(error(0, $agent), '', 'ajax');
}
