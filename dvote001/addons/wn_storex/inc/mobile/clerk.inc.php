...
if ($op == 'order') {
	...
	$manage_storex_lists = clerk_permission_storex($op, $storeid);
	...
	$operation_status = array(ORDER_STATUS_NOT_SURE, ORDER_STATUS_SURE);
	$goods_status = array(0, GOODS_STATUS_NOT_SHIPPED, GOODS_STATUS_SHIPPED, GOODS_STATUS_NOT_CHECKED);
	$order_lists = pdo_getall('storex_order', array('weid' => intval($_W['uniacid']), 'hotelid' => $manage_storex_ids, 'status' => $operation_status, 'goods_status' => $goods_status), array('id', 'weid', 'hotelid', 'paystatus','roomid', 'style', 'btime', 'etime', 'roomitemid', 'status', 'goods_status', 'mode_distribute', 'nums', 'sum_price', 'day', 'is_package', 'cart', 'cprice', 'oprice', 'ordersn'), '', 'id DESC');
	if (!empty($order_lists) && is_array($order_lists)) {
			...
			if (!empty($info['roomid'])) {
				...
				if (!empty($info['cart'])) {
					...
					foreach ($info['cart'] as &$g) {
						if ($g['buyinfo'][2] == 3) {
							$goods_info = pdo_get('storex_sales_package', array('uniacid' => $_W['uniacid'], 'id' => $g['buyinfo'][0]), array('title', 'sub_title', 'thumb', 'price', 'id'), 'id');
						} else {
							$goods_info = pdo_get('storex_goods', array('weid' => intval($_W['uniacid']), 'id' => $g['good']['id']), array('id', 'thumb'));
						}
		...
		$order_data['express'] = express_type();
					...
					if (isset($goods['is_house']) && $goods['is_house'] == 1) {
						...
						if (!empty($room_list) && is_array($room_list)) {
							foreach ($room_list as $r => $val) {
								$show = room_check_assign($order, $val['id']);
								if (empty($show)) {
									unset($room_list[$r]);
								}
							}
					...
					if (!empty($order['cart']) && is_array($order['cart'])) {
						foreach ($order['cart'] as $good) {
							if ($good['buyinfo'][2] == 3) {
								...
								$goods = pdo_get('storex_goods', array('id' => $good['good']['id'], 'weid' => $order['weid']), array('id', 'thumb', 'oprice', 'cprice', 'title', 'sub_title'));
...
if ($op == 'edit_order') {
	...
	if (!empty($_GPC['express_type'])) {
		$express = express_type($_GPC['express_type']);
		if (!empty($express)) {
			$data['express_type'] = $_GPC['express_type'];
			$data['track_number'] = $_GPC['track_number'];
			$data['express_name'] = $express;
		}
	}
	...
	if (!empty($data['status']) && $data['status'] != $item['status']) {
		...
		if ($data['status'] == ORDER_STATUS_OVER) {
			...
			//增加出售货物的数量
			add_sold_num($goods_info, $store_info[$item['hotelid']]['store_type']);
	...
	if (!empty($result)) {
		...
		if ($data['status'] == ORDER_STATUS_OVER) {
			order_market_gift($orderid);
			order_salesman_income($orderid, ORDER_STATUS_OVER);
			if ($store_info[$item['hotelid']]['store_type'] != STORE_TYPE_HOTEL) {
				order_goods_profit($orderid);//盈利记录
			}
		}
...
if ($op == 'room') {
	...
	if (!empty($room_list) && is_array($room_list)) {
		foreach ($room_list as $k => $info) {
			...
			$room_list[$k]['thumb'] = tomedia($info['thumb']);
		}
	}
...
if ($op == 'edit_room') {
	...
	if (!empty($dates) && is_array($dates)) {
		foreach ($dates as $date) {
			$roomprice = roomPrice($room_info['store_base_id'], $room_id, $date);
			$roomprice['num'] = $num;
...
if ($op == 'assign_room') {
	...
	if (!room_check_assign($order, $roomids, true)) {
	...
	if (!empty($result)) {
		if (!empty($assign_roomitemid) && is_array($assign_roomitemid)) {
			...
			foreach ($assign_roomitemid as $roomid) {
				room_delete_assign($order, $roomid);
			}
		}
...
if ($op == 'order_consume') {
	...
	if (!empty($_GPC['consume'])) {
		...
		$result = pdo_update('storex_order', array('status' => 3, 'over_time' => TIMESTAMP), array('id' => $orderid));
		if (!is_error($result)) {
			$logs = array(
				'table' => 'storex_order_logs',
				'time' => TIMESTAMP,
