...
if (in_array($op, array('address_post', 'address_default', 'address_delete')) && !empty($_GPC['id'])) {
	$address_info = pdo_get('storex_member_address', array('uniacid' => $_W['uniacid'], 'uid' => $uid, 'id' => intval($_GPC['id'])));
...
if ($op == 'personal_info') {
	...
	if (!empty($card_info)) {
		...
		foreach ($show_fields as $val) {
			...
			if ($val == 'background') {
				if (empty($card_info[$val])) {
					$card_info[$val]['background'] = 'system';
					$card_info[$val]['image'] = '1';
				}
	...
	$user_info['agent_status'] = 2;
	if (!empty($store_id)) {
		$store_info = get_store_info($store_id);
		$user_info['agent_status'] = $store_info['agent_status'];
	}
...
if ($op == 'address_lists') {
	$address_info = pdo_getall('storex_member_address', array('uid' => $uid, 'uniacid' => $_W['uniacid']), '', '', 'isdefault DESC');
	wmessage(error(0, $address_info), '', 'ajax');
}
if ($op == 'current_address') {
	$current_info = pdo_get('storex_member_address', array('id' => intval($_GPC['id']), 'uid' => $uid, 'uniacid' => $_W['uniacid']));
...
if ($op == 'address_post') {
	...
	if (!empty($_GPC['id'])) {
		$result = pdo_update('storex_member_address', $address_info, array('id' => intval($_GPC['id'])));
		wmessage(error(0, $result), '', 'ajax');
	} else {
		$address_info['uid'] = $uid;
		$address_info['uniacid'] = $_W['uniacid'];
		$address = pdo_get('storex_member_address', array('uniacid' => $_W['uniacid'], 'uid' => $uid));
		if (empty($address)) {
			$address_info['isdefault'] = 1;
		}
		$result = pdo_insert('storex_member_address', $address_info);
		wmessage(error(0, $result), '', 'ajax');
	}
}
if ($op == 'address_default') {
	$default_result = pdo_update('storex_member_address', array('isdefault' => '0'), array('uid' => $uid, 'uniacid' => $_W['uniacid']));
	$result = pdo_update('storex_member_address', array('isdefault' => '1'), array('id' => intval($_GPC['id'])));
	wmessage(error(0, '设置成功'), '', 'ajax');

}
if ($op == 'address_delete') {
	$result = pdo_delete('storex_member_address', array('id' => intval($_GPC['id'])));
...
if ($op == 'credit_password') {
	...
	$password = member_hash(trim($_GPC['password']), $member['credit_salt']);
...
if ($op == 'set_credit_password') {
	...
	if (!empty($member['password_lock'])) {
		...
		if (empty($string) || $string != $str) {
			wmessage(error(-1, '验证不通过,请重新验证'), '', 'ajax');
		}
	}
	...
	$salt = random(8);
	$password = member_hash($password, $salt);
...
if ($op == 'footer') {
	...
	$footer = pdo_get('storex_homepage', array('uniacid' => $_W['uniacid'], 'storeid' => $storeid, 'type' => 'footer', 'is_wxapp' => $is_wxapp));
...
if ($op == 'set_password') {
	...
	if (in_array($type, array('phone', 'email'))) {
		...
		if (!empty($codes) && is_array($codes)) {
			...
			$salt = random(8);
			$update['credit_password'] = member_hash($password, $salt);
			$update['credit_salt'] = $salt;
			if ($type == 'phone') {
