...
$ops = array('order_list', 'order_detail', 'orderpay', 'check_group_order','cancel', 'confirm_goods', 'order_comment', 'refund', 'group_order', 'group_order_detail', 'single_express_info', 'cart_express_info');
...
if ($op == 'order_list') {
	...
	$field = array('id', 'weid', 'hotelid', 'roomid', 'style', 'nums', 'cprice', 'oprice', 'sum_price', 'status', 'paystatus', 'paytype', 'mode_distribute', 'goods_status', 'track_number', 'express_name', 'is_package', 'cart', 'comment');
	...
	if (!empty($orders) && is_array($orders)) {
		...
		foreach ($orders as $k => &$info) {
					...
					if (!empty($info['roomid'])) {
						...
						if (!empty($info['cart'])) {
							...
							foreach ($info['cart'] as &$g) {
								$goods_info = order_cart_goodsinfo($g);
								$g['good']['thumb'] = tomedia($goods_info['thumb']);
			...
			if ($info['mode_distribute'] != 2 && $info['status'] != ORDER_STATUS_CANCEL && $info['status'] != ORDER_STATUS_REFUSE && $info['status'] != ORDER_STATUS_OVER && empty($info['group_goodsid'])) {
...
if ($op == 'order_detail') {
	...
	$store_info = pdo_get('storex_bases', array('weid' => intval($_W['uniacid']), 'id' => $order_info['hotelid']), array('id', 'title', 'store_type', 'return_info'));
	...
	if ($order_info['store_type'] == 1) {
		...
		if (!empty($order_info['roomid'])) {
			if ($order_info['is_package'] == 2) {
				$goods_info = pdo_get('storex_sales_package', array('uniacid' => $_W['uniacid'], 'id' => $order_info['roomid']), array('title', 'sub_title', 'thumb', 'price', 'id'));
			...
			if ($order_info['is_package'] == 1) {
				$goods_info['is_return'] = order_check_return($order_info);
			}
			$order_goods[] = $goods_info;
		} else {
			...
			if (!empty($order_info['cart']) && is_array($order_info['cart'])) {
				foreach ($order_info['cart'] as $good) {
					$goods_info = order_cart_goodsinfo($good);
					...
					$goods_info['thumb'] = tomedia($goods_info['thumb']);
					$goods_info['buyinfo'] = implode('|', $good['buyinfo']);
					$goods_info['is_return'] = order_check_return($order_info, $good);
					$order_goods[] = $goods_info;
				}
			}
		}
	...
	if (!empty($order_info['addressid'])) {
		$order_address = pdo_get('storex_member_address', array('uid' => $uid, 'uniacid' => intval($_W['uniacid']), 'id' => $order_info['addressid']));
	...
	$order_info['return_set'] = iunserializer($store_info['return_info']);
...
if ($op == 'check_group_order') {
	...
	if (pdo_fieldexists('storex_order', 'group_id')) {
		...
		if (!empty($order['group_id'])) {
			$result = plugin_group_status($order['group_id']);
			if (is_error($result)) {
...
if ($op == 'order_comment') {
	...
	if ($order_info['comment'] == 0) {
		if ($order_info['status'] == 3 || $order_info['goods_status'] == GOODS_STATUS_CHECKED) {
...
if ($op == 'group_order') {
	...
	$orders = pdo_getall('storex_order', array('openid' => $_W['openid'], 'group_goodsid !=' => 0), $fields, '', 'time DESC');
	if (!empty($orders)) {
		$storex_bases = array();
		$group_goodsids = array();
		$goodsids = array();
		$group_ids = array();
        $storex_bases_ids = array();
...
if ($op == 'group_order_detail') {
	...
	$order_info = pdo_get('storex_order', array('id' => $orderid, 'openid' => $_W['openid'], 'group_goodsid !=' => 0));
	if (!empty($order_info)) {
		$order_info['time'] = date('Y-m-d', $order_info['time']);
		if (!empty($order_info['mode_distribute'])) {
			$order_info['order_time'] = date('Y-m-d', $order_info['order_time']);//自提或配送时间
		...
		if (!empty($group)) {
			...
			$head_info = mc_fansinfo($order_info['openid']);
		}
...
if ($op == 'single_express_info') {
	$citycode = intval($_GPC['citycode']);
	$goodsid = intval($_GPC['goodsid']);
	$areadata = express_get_areadata();
    $city_code = '';
	...
	$freight = express_single_info($goodsid, $city_code);
	wmessage(error(0, $freight), '', 'ajax');
}

if ($op == 'cart_express_info') {
	$storeid = intval($_GPC['storeid']);
	$citycode = intval($_GPC['citycode']);
	...
	$freight = express_cart_info($goodsids, $storeid, $city_code);
	wmessage(error(0, $freight), '', 'ajax');
}
