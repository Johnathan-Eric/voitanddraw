...
if ($op == 'display') {
	...
	$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('storex_clerk') . " WHERE `weid` = :weid AND `storeid` = :storeid " . $sql, $params);
...
if ($op == 'pay_record') {
	...
	$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('storex_clerk_pay') . " WHERE clerkid = :clerkid AND uniacid = :uniacid", array(':clerkid' => $id, ':uniacid' => $_W['uniacid']));
	$pager = pagination($total, $pindex, $psize);
	include $this->template('store/shop_clerk');
}