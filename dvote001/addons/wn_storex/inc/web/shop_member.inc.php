...
$ops = array('display', 'edit', 'delete', 'deleteall', 'showall', 'status', 'clerk', 'cost_record', 'assign_member_group');
...
if ($op == 'display') {
	...
	if (!empty($member_list)) {
		foreach ($member_list as &$m) {
			if (!empty($m['member_group'])) {
				$m['member_group'] = iunserializer($m['member_group']);
			}
		}
	}
	$params[':weid'] = $_W['uniacid'];
	$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('storex_member') . " WHERE weid = :weid " . $sql, $params);
	$pager = pagination($total, $pindex, $psize);
	$memberlevel = pdo_getall('storex_member_level', array('uniacid' => intval($_W['uniacid']), 'storeid' => $storeid), array(), 'id', 'level ASC');
	...
	$list = pdo_getall('storex_member', array('weid' => $_W['uniacid']), array('id', 'createtime'));
	if (!empty($list) && is_array($list)) {
		foreach ($list as $value) {
			if (!empty($value['createtime']) && $value['createtime'] >= $starttime && $value['createtime'] <= $endtime) {
				$stat[date('Ymd', $value['createtime'])] += 1;
			}
		}
	}
...
if ($op == 'edit') {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($insert['password'])) {
			...
			$insert['password'] = member_hash($_GPC['password'], $insert['salt']);
...
if ($op == 'assign_member_group') {
	$memberid = intval($_GPC['memberid']);
	$member_level_id = intval($_GPC['member_level_id']);
	if (empty($memberid) || empty($member_level_id)) {
		message(error(-1, '参数错误'), '', 'ajax');
	}
	$member = pdo_get('storex_member', array('id' => $memberid));
	if (empty($member)) {
		message(error(-1, '用户不存在'), '', 'ajax');
	}
	$member_level = pdo_get('storex_member_level', array('id' => $member_level_id, 'uniacid' => intval($_W['uniacid']), 'storeid' => $storeid), array(), 'id', 'level ASC');
	if (empty($member_level)) {
		message(error(-1, '会员组不存在'), '', 'ajax');
	}
	if (!empty($member['member_group'])) {
		$member['member_group'] = iunserializer($member['member_group']);
		
	}
	$member['member_group'][$storeid] = $member_level_id;
	if (pdo_update('storex_member', array('member_group' => iserializer($member['member_group'])), array('id' => $member['id']))) {
		message(error(0, '分配会员组成功'), '', 'ajax');
	} else {
