			...
			if ($value['type'] == 'recommend') {
				if ($_W['wn_storex']['store_info']['store_type'] != STORE_TYPE_HOTEL) {
					if (!empty($value['items']) && is_array($value['items'])) {
						$goodslist = pdo_getall($_W['wn_storex']['goods_table'], array('id' => array_values($value['items'])), array('id', 'thumb', 'title', 'cprice'), 'id');
...
if ($op == 'link') {
	if ($_W['ispost'] && $_W['isajax']) {
		...
		if (plugin_exists('wn_storex_plugin_bargain') && empty($_W['wn_storex']['store_info']['store_type'])) {
			$entries[] = array(
				'type' => 'bargain',
				'name' => '砍价',
				'link' => '/Bargain/Index',		
