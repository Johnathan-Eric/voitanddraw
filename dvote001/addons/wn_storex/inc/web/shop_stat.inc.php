...
global $_W, $_GPC;
$ops = array('order_stat', 'sales_stat', 'profit_stat');
...
if ($op == 'profit_stat') {
	...
	$profit_list = pdo_getall('storex_goods_profit', array('uniacid' => $_W['uniacid'], 'storeid' => $storeid, 'order_time >=' => $starttime, 'order_time <' => $endtime));
	$total_profit = 0;
	if (!empty($profit_list) && is_array($profit_list)) {
		foreach ($profit_list as $profit) {
			$date = date('Ymd', $profit['order_time']);
			$profit_stat[$date] += $profit['money'];
			$total_profit += $profit['money'];
		}
	}
	$chart_data['label'] = array_keys($profit_stat);
	$chart_data['series_data'] = array_values($profit_stat);
	include $this->template('store/shop_stat_profit');
}
