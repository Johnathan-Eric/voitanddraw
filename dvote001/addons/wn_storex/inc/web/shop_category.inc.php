...
if ($op == 'display') {
	...
	if (!empty($_GPC['displayorder'])) {
		...
		itoast('分类排序更新成功！', $this->createWebUrl('shop_category', array('op' => 'display', 'storeid' => $storeid)), 'success');
	}
	$children = array();
...
if ($op == 'post') {
	...
	if (!empty($parentid)) {
		...
		if (empty($parent)) {
			itoast('上级分类不存在或是已经被删除！', 'refresh', 'error');
		}
	}
	if (checksubmit('submit')) {
		if (empty($_GPC['name'])) {
			itoast('请输入分类名称！', 'refresh', 'error');
		}
