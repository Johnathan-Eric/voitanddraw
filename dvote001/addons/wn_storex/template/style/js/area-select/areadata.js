...
strVarCity += '                      <button class="aui-btn aui-btn-primary" onclick="save_City()" type="button">确定</button>';
...
$(document).on('click', '.js-select-area', function() {
	appendCity(this, 'duoxuan', 'normal');
});
$(document).on('click', '.js-select-special-area', function() {
	appendCity(this, 'duoxuan', 'special');
});
function appendCity(thiscon, Cityxz, type) {
	dataCityinput = thiscon;
	datatype = Cityxz;
	selecttype = type;
	$('.js-main').append(strVarCity);
	if (datatype == "danxuan") {
	...
	if ($(dataCityinput).data("value") != "") {
		if (selecttype == 'special') {
			// var inputarryname = $(dataCityinput).val().split('-');
			var inputarryname = $(dataCityinput).parent().find('.js-show-special-area').text().split('-');
			var inputarry = $(dataCityinput).next('input[name="selectedareas_code"]').val().split('-');
		} else if (selecttype == 'normal') {
			var inputarry = $(dataCityinput).next('input[name="city_area[]"]').val().split('-');
			var inputarryname = $(dataCityinput).text().split('-');
		}
...
function save_City() {
	...
	if (selecttype == 'normal') {
		$(dataCityinput).next('input[name="city_area[]"]').val(val);
		$(dataCityinput).text(Cityname);
	} else if (selecttype == 'special') {
		$(dataCityinput).next('input[name="selectedareas_code"]').val(val);
		// $(dataCityinput).val(Cityname);
		$(dataCityinput).parent().find('.js-show-special-area').text(Cityname);
	}
