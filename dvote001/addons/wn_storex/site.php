...
class Wn_storexModuleSite extends WeModuleSite {
	...
	public function get_wxapp_appid() {
		global $_W, $_GPC;
	...
	public function doMobiledisplay() {
		...
		$color = '';
		if (!empty($id)) {
			$storex_bases = pdo_get('storex_bases', array('id' => $id), array('id', 'color'));
			if (!empty($storex_bases)) {
				$color = $storex_bases['color'];
			}
		}
		if (!empty($id) && !empty($_GPC['type'])) {
			$params = array(
				...
				'bargain_list_id' => $_GPC['bargain_list_id'],
			);
			mload()->model('entry');
		...
		if ($setting['version'] == 0 && empty($id) && empty($_GPC['v'])) {
	...
	public function payResult($params) {
		...
		if (!empty($recharge_info)) {
			...
			$storex_bases = pdo_get('storex_bases', array('id' => $order['hotelid'], 'weid' => $weid), array('id', 'store_type', 'title', 'emails', 'phones', 'openids', 'mail', 'template'));
			...
			if ($params['from'] == 'return') {
				...
				if ($params['result'] == 'success') {
					if ($_W['account']['type'] != 4 || $_W['account']['uniacid'] == $_W['uniacid']) {
						//发送模板消息提醒
						if (!empty($template['template']) && !empty($template['confirm_templateid'])) {
							$time = '';
							...
							$account_api->sendTplNotice($_W['uniacid'], $template['confirm_templateid'], $data);
						...
						if ($storex_bases['store_type'] == STORE_TYPE_HOTEL && !empty($template['template']) && !empty($template['templateid'])) {
							$tplnotice = array(
								...
								'remark' => array('value' => '为保证用户体验度，请及时处理！ 支付状态:已支付！')
							);
							foreach ($clerks_openids as $clerk) {
								$account_api->sendTplNotice($clerk, $template['templateid'], $tplnotice);
							}
						} else {
							foreach ($clerks_openids as $clerk) {
								$info = '店铺有新的订单,为保证用户体验度，请及时处理! 支付状态:已支付！';
								$custom = array(
									'msgtype' => 'text',
									'text' => array('content' => urlencode($info)),
									'touser' => $clerk,
					...
					if (plugin_exists('wn_storex_plugin_printer')) {
						$mode_info = print_get_mode($order['hotelid']);
						if (!empty($mode_info['defaultid']) && in_array('2', $mode_info['print_mode'])) {
							print_order($mode_info['defaultid'], $order['id'], $order['hotelid']);
						}
					}
				}
