...
class OrdersLogic
{
    ...
    function get_orders_goods($order_id = 0)
    {
        ...
        $model-> alias("o")
            -> join("left join ".C("DB_PREFIX")."goods_spec gs on gs.spec_id = o.spec_id")
            -> field("o.*,gs.goods_stock")
            ->where($map);
        $lists = $model->select();
        return $lists;
    }
...
//                 @D("Common/Message", "Logic")->send_wx_message($data['uid'], $tmp_data, 'm_order_shipped');
    ...
    */
    ...
    {
        $model = D("Common/orders");
        $join = " LEFT JOIN zy_orders_goods og ON og.order_id = o.order_id "
                . " LEFT JOIN zy_member m ON m.uid = o.uid";
        $alllists = $model->field('o.order_id')->alias('o')->where($where)->join($join)->group('o.order_id')->select();
    ...
    function count_order()
    {
        ...
        $status4Where = $where;
        $status4Where['order_status'] = array('eq','4');  //已完成订单
        $return['status4_num_count'] = $this->num_count($status4Where); //总订单
        $status1Where = $where;
        $status1Where['order_status'] = array('eq','1');  //待发货订单
        $return['status1_num_count'] = $this->num_count($status1Where); //总订单
        $status5Where = $where;
        $status5Where['order_status'] = array('eq','5');  //待处理退货订单
        $return['status5_num_count'] = $this->num_count($status5Where); //总订单
        $status6Where = $where;
        $status6Where['order_status'] = array('eq','6');  //待确认退货订单
    ...
    {
        $model = D("Common/orders");
        $info = $model->field("sum(order_amount) as total")->where($where)->find(); //订单总额
        return $info['total'];
    }
     ...
     * 配置收藏数据
     * ****/
    static function fav_get_date($parm)
    {
        $data = array();
        $isAdd = 1;
        $data['type'] = $parm['type'];
        $data['goods_id'] = $parm['goods_id'];
        if ($parm['entity_id']) {
            $data['entity_id'] = $parm['entity_id'];
        }

        //通过type获取不同分支参数
        switch ($parm['type']){
            case '1':
                $groupInfo = M('GroupBuying')
                            ->field('id,activity_name,status,imgs')
                            ->find($parm['type_id']);
                $imgs = explode(',', $groupInfo['imgs']);
                $isAdd = $groupInfo ? 1 : 0;
                $data['goods_picture'] = HTTP_ADDRESS . $imgs[0];
                if ($groupInfo['status'] == 3) 
                {
                    $data['is_online'] = 0;
                }else{
                    $data['is_online'] = 1;
                }
                $data['favtitle'] = $parm['favtitle']."【团购】";
                break;
            case '2':
                $info = M('Package')->field('pa_id,name,status,imgs')->where(array('pa_id'=>$parm['goods_id']))->find();
                ...
                $data['favtitle'] = $parm['favtitle']."【套餐】";
                break;
            case '3':
                $info = M('Integral_goods')->alias("ig")
                                ->field("ig.*,b.book_name,b.book_sn,b.price,b.book_picture")
                                ->join(" left join ".C("DB_PREFIX")."books b on ig.goods_id = b.book_id ")
                                ->where(array('ig.id'=>$parm['type_id']))
                                ->find();
                $imgs = array();
                empty($info['img_url']) ? $imgs = explode(',', $info['book_picture']) : $imgs[0] = $info['img_url'];
                $isAdd = $info ? 1 : 0;
                $data['goods_picture'] = HTTP_ADDRESS . $imgs[0];
                if ($groupInfo['status'] == 3) 
                {
                ...
                $data['favtitle'] = $parm['favtitle']."【积分兑换商品】";
                break;
            default :
                $bookInfo = M('Books')
                            ->field('book_id,book_sn,book_name,book_picture,is_online')
                            ->find($parm['goods_id']);
                $isAdd = $bookInfo ? 1 : 0;
                $data['goods_picture'] = HTTP_ADDRESS . $bookInfo['book_picture'];
                $data['is_online'] = $bookInfo['is_online'];
                $data['favtitle'] = $parm['favtitle']."【商品】";
                break;
        }
        return $data;
    }
     ...
     * @return blooen true/false
     **/
    public function saveGbuyingOrder($order_id) {
        if (!$order_id) {
            return false;
        }

        // 根据订单ID获取拼团信息
        $order = M('Orders')->where(array('order_id' => array('eq', $order_id)))->field('act_id,act_gid')->find();
        if (empty($order)) {
            return false;
        }

        // 查看拼团数据
        $gbInfo = M('GroupBuying')->where(array('id' => $order['act_id']))->field('num')->find();
        if (empty($gbInfo)) {
            return false;
        }

        // 根据act_gid查询订单总数
        $orderNum = M('Orders')->where(array('act_gid' => $order['act_gid'], 'order_status' => 7))->count();
        if ($orderNum >= $gbInfo['num']) {
            // 更新拼团订单状态为待发货
            M('Orders')->where(array('act_gid' => $order['act_gid']))->save(array('order_status' => 1));
            return true;
        } else {
            return false;
        }
