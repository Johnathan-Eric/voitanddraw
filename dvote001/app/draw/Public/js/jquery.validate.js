	...
	validate: function( options ) {
		...
		if ( validator.settings.onsubmit ) {
			...
			if (validator.settings.submitHandler) {
				this.find("input, button").filter("[type='submit']").click(function() {
...
$.extend($.validator, {
	...
	methods: {
			...
			$.ajax($.extend(true, {
				...
				success: function(response) {
					...
					if ( valid ) {
						...
						var errors = {};
						var remote_msg;
						var message = response.valid || (response.msg ? response.msg : validator.defaultMessage( element, "remote" ));
		...
		time: function(value, element) {
			return this.optional(element) || /^(([1-9]{1})|([0-1][0-9])|([1-2][0-3])):([0-5][0-9])$/.test(value);
		},
		...
		undigits: function(value, element) {
			return this.optional(element) || !/^\d+$/.test(value);
		},
		...
		telphone: function(value, element) {
			return this.optional(element) || /^0\d{2,3}(\-)?\d{7,8}$/.test(value) || /^1[3|4|5|8][0-9]\d{8}$/.test(value);
		},
		...
		idcard: function(value, element) {
			...
			if (area[parseInt(idcard.substr(0, 2))] == null){error = Errors[4];return false;
			}
...
$(document).ready(function($){
    $("#myform,.myform").validate({
    			...
    			errorPlacement: function(error, element) {
    				error.appendTo(element.parent("div,td,span,label") );
    			},
