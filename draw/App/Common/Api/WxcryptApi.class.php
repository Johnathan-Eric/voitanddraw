...
class WxcryptApi
{
	...
	public function set_config($config = array())
	{
		...
		$this->m_sEncodingAesKey  = $config['aeskey'];
	...
	*/
	public function VerifyURL($sMsgSignature, $sTimeStamp, $sNonce, $sEchoStr, &$sReplyEchoStr)
	{
		if (strlen($this->m_sEncodingAesKey) != 43) {
			return Weixin\ErrorCode::$IllegalAesKey;
		}

		$pc = new \Common\Api\Weixin\Prpcrypt($this->m_sEncodingAesKey);
		//verify msg_signature
		$sha1 = new \Common\Api\Weixin\SHA1;
		...
		if ($signature != $sMsgSignature) {
			return Weixin\ErrorCode::$ValidateSignatureError;
		}
	...
	public function EncryptMsg($sReplyMsg, $sTimeStamp, $sNonce, &$sEncryptMsg)
	{
		...
		$pc = new \Common\Api\Weixin\Prpcrypt();
		...
		$signature = $array[1];
		//生成发送的xml
		$xmlparse = new \Common\Api\Weixin\XMLParse;
