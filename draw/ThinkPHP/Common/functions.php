...
function D($name='',$layer='',$tb_name = "") {
    ...
    if(class_exists($class)) {
        $model      =   new $class(basename($tb_name ? $tb_name : $name));
    ...
    $_model[$name.$layer.$tb_name]  =  $model;
    return $model;
}
...
function A($name,$layer='',$level='') {
    ...
    //$action = new Admin\Action\CashAction();
    if(class_exists($class)) {
        $action             =   new $class();
...
function U($url='',$vars='',$suffix=true,$domain=false) {
    // 解析URL
    $info   =  parse_url($url);
    if(is_numeric($info['path'])){
        //2014-12-7 如果action是数字。跳过  @author wscsky
        $info['path'] = "index";
        C("REPLACE_URL_INDEX",true);
    }
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if(!empty($vars)) { // 添加参数
            foreach ($vars as $var => $val){
                if('' !== trim($val)){
                	//if为修改版 wscsky
                	if(C("SHORT_URL_ON") && in_array($var, C("SHORT_URL_SKIP"))){
    ...
    if(C("REPLACE_URL_INDEX") || C('URL_TRIM_INDEX_ACT')){
        $url = str_ireplace("/index/", "/", $url);
        C("REPLACE_URL_INDEX",false);
    }
...
* @author wscsky
*/ 
function imagebmp(&$im, $filename = "", $bit = 32, $compression = 0) {
