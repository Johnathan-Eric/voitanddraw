...
function add_uploadok(data)
{
	...
	var imgsrc 	= data.filepath;
	var thumb	= data.thumb ? data.thumb : data.filepath;
	var ext 	= data.isimage;
	var name 	= data.filename;
	var filesize= data.filesize;
	var aids 	= data.aids;

	var fext = thumb.substring(thumb.length - 4);

	if(fext == ".jpg" || fext == ".gif" || fext == ".png" || fext == ".bmp"){ext = 1;}

	if(ext == 1) {
		var img = '<a href="javascript:void(0);" onclick="javascript:add_file(this,'+id+',\''+aids+'\')" id="on_'+id+'" class="on"><div class="icon"></div><img src="'+thumb+'" width="80" alt="'+name+'" imgid="'+id+'" path="'+src+'"/></a>';
	} else {
		var img = '<a href="javascript:void(0);" onclick="javascript:add_file(this,'+id+',\''+aids+'\')" id="on_'+id+'" class="on"><div class="icon"></div><img src="Public/images/ext/'+ext+'.png" width="80" alt="'+name+'" imgid="'+id+'" path="'+src+'"/></a>';
	}
...
function add_file(obj,id,status,aids){
